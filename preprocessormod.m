...
function [coord,centelem,elem,esurn1,esurn2,nsurn1,nsurn2,bedge,inedge,...
    ...
    kmap,wells,klb,limiterflag,rowposit,nltol,maxiter,acel,midflowcompared] = preprocessormod
...
for ielem = 1:nelem
    ...
    for inode = 1:amountedges
        ...
        if any(i)
            ...
            %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            %============================= modificamos
            % is used when the cell is quase convex
            if abs((angbtweenedges/180)-1)<10^-1 && min(cross(vecnext,vecbehind))<0
            pointsign = cross(vecbehind,vecnext);
            %=============================================================
            else
